...
8. **Migas en la cabecera.** La marca muestra el nombre de la escuela salvo que una página fije migas con `SetHeaderCrumbs` (`src/components/header-title.tsx`), pintadas con `Breadcrumb` y el patrón "breadcrumb con dropdown" de shadcn: el vídeo pone "Curso / Clase / dd-mm-aa"; curso y clase abren un `DropdownMenu` con todos los cursos o todas las clases del curso (con su fecha) y la actual marcada con un check. Van en una sola línea: si no cabe, se recorta con "…" primero el curso (`shrink-[4]`), luego la clase (`shrink-[1]`), y la fecha nunca (`shrink-0`).
...
- **Controles solo con símbolos**: reproducir/pausa (`Button` de icono), sonido (`Toggle` con altavoz) y velocidad: un `Button` con el icono de dos triángulos que abre un `Popover` con un `Slider` de 0,5× a 1,5×, como un control de volumen; cuando no es 1× el botón muestra el valor.
